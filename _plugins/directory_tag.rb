...
module Jekyll
  ...
  class DirectoryTag < Liquid::Block
    ...
    def render(context)
	  ...
	  basename_no_ext = File.basename(filename, ".*")

	  #Procedura per nome con estensione
          filepath  = [@path, basename] - ['.']
          path = filepath.join '/'
          url  = '/' + filepath.join('/')

	  #Procedura per nome senza estensione
	  filepath_no_ext  = [@path, basename_no_ext] - ['.']
          path_no_ext = filepath_no_ext.join '/'
          url_no_ext  = '/' + filepath_no_ext.join('/')
